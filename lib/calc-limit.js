...
export async function verificarLimiteCalc(supabase, userId) {
  ...
  const limiteCalc = planConfig.limits.calculadora.monthly
  const limite = limiteCalc ?? Infinity
  ...
  if (limite !== Infinity && calcsEsteMes >= limite) {
    const label = planConfig.limits.calculadora.label
    return {
      permitido: false,
      motivo: `Alcanzaste el límite de ${label} del plan ${plan === 'free' ? 'gratuito' : plan}. Actualizá a Pro para cálculos ilimitados.`,
      calcsEsteMes,
      limite,
      limitAlcanzado: true,
    }

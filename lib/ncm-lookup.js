...
// Aplana los joins de aranceles_importacion y aranceles_exportacion
...
function enriquecerRow(r) {
  ...
  const ae = Array.isArray(r.aranceles_exportacion)
    ? (r.aranceles_exportacion[0] ?? {})
    : (r.aranceles_exportacion ?? {})
 ...
 * @returns {Promise<Array<{codigo_ncm: string, descripcion: string, arancel_extrazona: number, derecho_exportacion: number, _fuenteBusqueda: string}>>}
 */
...
export function formatearResultadosNCM(resultados, { esNaladisa = false } = {}) {
  ...
  const filas = resultados.map((r) => {
    ...
    if (r.reintegro) lineas.push(`  Reintegro: ${r.reintegro}%`)
    return lineas.join('\n')
  })

...
// ─── Tarifa courier puerta a puerta (FedEx International Economy, importación) ──
// El régimen courier NO paga flete marítimo/aéreo + despachante: paga una tarifa
// puerta a puerta que cobra el operador (FedEx/DHL), por zona de origen y peso.
// Fuente: FedEx Guía de Servicios y Tarifas Argentina 2026 (tarifas EN USD).
// No incluye recargo por combustible ni impuestos (el operador los cobra aparte).
const COURIER_PESO_DEFAULT = 3   // kg asumidos cuando no se ingresa peso
const COURIER_ZONA_DEFAULT = 'D' // zona usada para orígenes no mapeados

// País de origen (ISO3) → zona FedEx (columna "Otros servicios" del tarifario).
const COURIER_ZONA_FEDEX = {
  // Zona A — Mercosur / regional
  BRA: 'A', CHL: 'A', URY: 'A', PRY: 'A',
  // Zona B — resto de Sudamérica
  PER: 'B', BOL: 'B', COL: 'B', VEN: 'B', GUY: 'B', SUR: 'B', GUF: 'B',
  // Zona C — Norteamérica
  USA: 'C', MEX: 'C', CAN: 'C', PRI: 'C',
  // Zona D — Europa
  DEU: 'D', ESP: 'D', ITA: 'D', FRA: 'D', GBR: 'D', NLD: 'D', PRT: 'D', BEL: 'D',
  SWE: 'D', CHE: 'D', POL: 'D', AUT: 'D', DNK: 'D', IRL: 'D', FIN: 'D', NOR: 'D',
  GRC: 'D', CZE: 'D', HUN: 'D', ROU: 'D', RUS: 'D', LUX: 'D', EST: 'D', LVA: 'D',
  LTU: 'D', SVK: 'D', SVN: 'D',
  // Zona G — Asia
  CHN: 'G', JPN: 'G', KOR: 'G', HKG: 'G', TWN: 'G', SGP: 'G', THA: 'G', VNM: 'G',
  IDN: 'G', MYS: 'G', MAC: 'G',
}

// Tarifa Economy importación (USD) por zona, en puntos de peso reales (kg).
// Se interpola linealmente entre anclas; sobre 20 kg se usa el adicional por kg.
const FEDEX_ECONOMY_ANCHORS_KG = [0.5, 1, 2, 3, 5, 7, 10, 15, 20]
const FEDEX_ECONOMY_USD = {
  //   0.5      1        2        3        5        7        10       15       20
  A: [216.30, 233.40, 267.60, 301.90, 370.70, 426.70, 510.70, 669.90, 669.90],
  B: [270.70, 304.00, 370.60, 438.80, 578.40, 675.60, 821.40, 1037.40, 1037.40],
  C: [273.00, 317.70, 407.10, 496.00, 672.80, 787.20, 958.80, 1134.00, 1134.00],
  D: [352.70, 409.20, 522.20, 634.60, 858.20, 1002.60, 1219.20, 1448.20, 1564.50],
  E: [380.60, 440.60, 560.60, 679.70, 916.10, 1119.70, 1425.10, 1786.10, 2147.10],
  F: [450.50, 517.10, 650.30, 780.30, 1033.90, 1264.30, 1609.90, 1992.90, 2127.30],
  G: [384.50, 445.60, 567.80, 690.20, 935.40, 1089.80, 1321.40, 1379.40, 1492.40],
}
// USD por kg adicional sobre 20 kg (tramo 21-44 kg del tarifario).
const FEDEX_ECONOMY_PER_KG = { A: 31.90, B: 49.40, C: 54.00, D: 74.50, E: 102.30, F: 101.30, G: 71.70 }

// Estima el flete courier puerta a puerta (USD) según país de origen y peso.
function estimarFleteCourier(pais_origen_iso3, peso_kg) {
  const zona = COURIER_ZONA_FEDEX[pais_origen_iso3] ?? COURIER_ZONA_DEFAULT
  const tabla = FEDEX_ECONOMY_USD[zona] ?? FEDEX_ECONOMY_USD[COURIER_ZONA_DEFAULT]
  const anclas = FEDEX_ECONOMY_ANCHORS_KG
  const ultimo = anclas.length - 1
  const peso = Math.max(0.5, Number(peso_kg) || COURIER_PESO_DEFAULT)
  if (peso <= anclas[0]) return tabla[0]
  if (peso >= anclas[ultimo]) {
    const perKg = FEDEX_ECONOMY_PER_KG[zona] ?? FEDEX_ECONOMY_PER_KG[COURIER_ZONA_DEFAULT]
    return r(tabla[ultimo] + (peso - anclas[ultimo]) * perKg)
  }
  for (let i = 1; i <= ultimo; i++) {
    if (peso <= anclas[i]) {
      const t = (peso - anclas[i - 1]) / (anclas[i] - anclas[i - 1])
      return r(tabla[i - 1] + t * (tabla[i] - tabla[i - 1]))
    }
  }
  return tabla[ultimo]
}
...
function regimenCourierComercial({ valor_fob, cif }) {
  const warnings = []
  if (valor_fob > 3000) {
...
function regimenCourierPersonal({ valor_fob, cif, peso_kg }) {
...
export async function calcularImportacion(supabase, params) {
  ...
  // Flete estimado cuando no se ingresó: courier usa la tarifa puerta a puerta
  // (FedEx) por zona/peso; el resto usa tarifa por kg o % del FOB según el modo.
  ...
  if (!flete_internacional || flete_internacional === 0) {
    if (esCourier) {
      const pesoCourier = peso_kg || COURIER_PESO_DEFAULT
      const zona = COURIER_ZONA_FEDEX[pais_origen_iso3] ?? COURIER_ZONA_DEFAULT
      fleteEfectivo = estimarFleteCourier(pais_origen_iso3, pesoCourier)
      fleteEstimado = true
      notas.push(peso_kg
        ? `Flete courier puerta a puerta (FedEx Economy, zona ${zona}): ${peso_kg} kg → USD ${fleteEfectivo}`
        : `Flete courier puerta a puerta (FedEx Economy, zona ${zona}) estimado para ${COURIER_PESO_DEFAULT} kg = USD ${fleteEfectivo} — ingresá el peso para mayor precisión`)
    } else if (peso_kg) {
      const modoEfectivo = modo ?? 'maritimo'
  ...
  if (regimen === 'courier_comercial') {
    const resultado = regimenCourierComercial({ valor_fob, cif })
    return {
      regimen_unico: true,
  ...
  if (regimen === 'courier_personal') {
    const resultado = regimenCourierPersonal({ valor_fob, cif, peso_kg })
    return {
      regimen_unico: true,
      regimen,
      ncm,

...
// Tasas planas fijas: 20% DI, 3% TE, 21% IVA — sin percepciones
...
function regimenCourierComercial({ valor_fob, cif }) {
  ...
  const DI_PCT = 20, TE_PCT = 3, IVA_PCT = 21
  ...
  return {
    ...
    desglose: {
      derecho_importacion: { alicuota: DI_PCT, monto: derecho, nota: 'Tasa plana courier' },
      tasa_estadistica:    { alicuota: TE_PCT, monto: tasaEst,  nota: 'Tasa plana courier' },
...
// Franquicia USD 400 exenta; sobre el exceso: 20% DI, 3% TE, 21% IVA
// Límite FOB USD 3.000.
function regimenCourierPersonal({ valor_fob, cif }) {
...
export async function calcularImportacion(supabase, params) {
  ...
  if (regimen === 'courier_comercial') {
    ...
    return {
      ...
      ncm,
      valores_base: { fob: valor_fob, flete: flete_internacional, seguro, cif },
  ...
  if (regimen === 'courier_personal' || regimen === 'puerta_a_puerta') {
    const resultado = regimenCourierPersonal({ valor_fob, cif })
    return {
      regimen_unico: true,
      regimen,
      ncm,

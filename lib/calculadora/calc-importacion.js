...
// Aranceles reales del NCM — sin percepciones
// Base imponible: CIF. Límite FOB USD 3.000.
function regimenCourierComercial({ valor_fob, cif, arancel, tasaEstadistica, iva }) {
  const warnings = []
  if (valor_fob > 3000) {
    warnings.push('El valor FOB supera el límite de USD 3.000 para courier comercial')
  }
  const DI_PCT = arancel
  const TE_PCT = tasaEstadistica
  const IVA_PCT = iva
  const derecho     = r(cif * (DI_PCT / 100))
  const tasaEst     = r(cif * (TE_PCT / 100))
  const baseIva     = r(cif + derecho + tasaEst)
  const ivaMonto    = r(baseIva * (IVA_PCT / 100))
  ...
  return {
    ...
    desglose: {
      derecho_importacion: { alicuota: DI_PCT, monto: derecho, nota: null },
      tasa_estadistica:    { alicuota: TE_PCT, monto: tasaEst,  nota: null },
      iva:                 { alicuota: IVA_PCT, monto: ivaMonto, nota: null },
    },
    total_tributos: total,
    costo_total:    r(cif + total),
    effective_rate: total > 0 ? r((total / cif) * 100) : 0,
...
// ─── Régimen Courier Personal / Puerta a Puerta ──────────────────────────────
// Franquicia USD 400 exenta; sobre el exceso: aranceles reales del NCM
// Límite FOB USD 3.000.
function regimenCourierPersonal({ valor_fob, cif, arancel, tasaEstadistica, iva }) {
  const FRANQUICIA = 400
  const warnings = []
  if (valor_fob > 3000) {
    warnings.push('El valor FOB supera el límite de USD 3.000 para courier personal')
  }
  const exceso = Math.max(0, cif - FRANQUICIA)
  const DI_PCT = arancel
  const TE_PCT = tasaEstadistica
  const IVA_PCT = iva
  const derecho  = r(exceso * (DI_PCT / 100))
  const tasaEst  = r(exceso * (TE_PCT / 100))
  const baseIva  = r(exceso + derecho + tasaEst)
  const ivaMonto = r(baseIva * (IVA_PCT / 100))
  const total    = r(derecho + tasaEst + ivaMonto)
  return {
    disponible: valor_fob <= 3000,
    motivo_no_disponible: valor_fob > 3000 ? 'Excede USD 3.000 FOB' : null,
    desglose: {
      franquicia:          { monto: Math.min(cif, FRANQUICIA), nota: 'Franquicia exenta USD 400' },
      derecho_importacion: { alicuota: DI_PCT, monto: derecho,  nota: `Sobre exceso USD ${r(exceso)}` },
      tasa_estadistica:    { alicuota: TE_PCT, monto: tasaEst,   nota: null },
      iva:                 { alicuota: IVA_PCT, monto: ivaMonto, nota: null },
    },
    total_tributos: total,
    costo_total:    r(cif + total),
    effective_rate: cif > 0 ? r((total / cif) * 100) : 0,
    warnings,
  }
}
...
export async function calcularImportacion(supabase, params) {
  ...
  // Flete courier estimado: solo para regímenes courier cuando no se ingresó flete
  const esCourier = regimen === 'courier_comercial' || regimen === 'courier_personal'
  let fleteEfectivo = flete_internacional
  if (esCourier && (!flete_internacional || flete_internacional === 0) && peso_kg) {
    const fleteCourier = r(peso_kg * 15)
    fleteEfectivo = fleteCourier
    notas.push(`Flete courier estimado: ${peso_kg} kg × USD 15/kg = USD ${fleteCourier}`)
  }
  ...
  if (regimen === 'courier_comercial') {
    const resultado = regimenCourierComercial({ valor_fob, cif, arancel, tasaEstadistica, iva })
    return {
      regimen_unico: true,
  ...
  if (regimen === 'courier_personal') {
    const resultado = regimenCourierPersonal({ valor_fob, cif, arancel, tasaEstadistica, iva })
    return {
      regimen_unico: true,
      regimen,
      ncm,
  ...
  if (regimen === 'puerta_a_puerta') {
    const resultado = regimenCourierPersonal({ valor_fob, cif, arancel: 20, tasaEstadistica: 3, iva: 21 })
    return {
      regimen_unico: true,
      regimen,
      ncm,

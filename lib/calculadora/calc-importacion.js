...
// ─── Tarifa courier puerta a puerta (consolidador) ──────────────────────────
// El régimen courier NO paga flete marítimo/aéreo + despachante: paga una tarifa
// puerta a puerta a un consolidador (AeroBox, TiendaMia, etc.), que junta cargas
// y cobra una tarifa PLANA por kg — mucho más barata que el courier express retail.
// Referencia: AeroBox China ≈ USD 25/kg puerta a puerta (20 kg → USD 500).
const COURIER_PESO_DEFAULT = 3            // kg asumidos cuando no se ingresa peso
const COURIER_FLETE_USD_KG_DEFAULT = 25   // USD/kg consolidador (ref: AeroBox China)
// Override de USD/kg por país de origen (ISO3). Sin entrada → usa el default.
// TODO: completar con tarifas reales del consolidador por origen.
const COURIER_FLETE_USD_KG = {
  CHN: 25, // AeroBox (confirmado)
}
...
export async function calcularImportacion(supabase, params) {
  ...
  // Flete estimado cuando no se ingresó: courier usa la tarifa de consolidador
  // puerta a puerta (USD/kg); el resto usa tarifa por kg o % del FOB según el modo.
  ...
  let fleteCourierUsdKg = null   // USD/kg usado para el envío courier (se muestra en el informe)

  if (!flete_internacional || flete_internacional === 0) {
    if (esCourier) {
      const pesoCourier = peso_kg || COURIER_PESO_DEFAULT
      const perKg = COURIER_FLETE_USD_KG[pais_origen_iso3] ?? COURIER_FLETE_USD_KG_DEFAULT
      fleteCourierUsdKg = perKg
      fleteEfectivo = r(perKg * pesoCourier)
      fleteEstimado = true
      notas.push(peso_kg
        ? `Flete courier puerta a puerta (consolidador): ${peso_kg} kg × USD ${perKg}/kg = USD ${fleteEfectivo}`
        : `Flete courier puerta a puerta (consolidador) estimado para ${COURIER_PESO_DEFAULT} kg × USD ${perKg}/kg = USD ${fleteEfectivo} — ingresá el peso para mayor precisión`)
    } else if (peso_kg) {
  ...
  // Courier: la tarifa puerta a puerta del operador es el costo de envío total —
  // no se le suma un seguro aparte (no inventamos costos que el operador no cobra).
  const seguro = seguro_internacional !== null
    ? seguro_internacional
    : (esCourier ? 0 : r((valor_fob + fleteEfectivo) * 0.01))
  ...
  if (seguro_internacional === null && !esCourier) notas.push(`Seguro estimado 1% de (FOB + flete) = USD ${seguro}`)
  ...
  if (regimen === 'courier_comercial') {
    ...
    return {
      ...
      ncm,
      valores_base: { fob: valor_fob, flete: fleteEfectivo, seguro, cif, flete_estimado: fleteEstimado, flete_usd_kg: fleteCourierUsdKg },

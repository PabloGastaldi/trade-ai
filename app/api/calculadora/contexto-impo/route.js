...
// GET /api/calculadora/contexto-impo?ncm=&pais=
// Devuelve acuerdos disponibles + barreras NTM argentinas para enriquecer
// el resultado de la calculadora de importación.
...
export async function GET(request) {
  ...
  if (!ncmRaw || !paisISO3) {
    return NextResponse.json({ acuerdos: [], ntm: [] })
  }

  const normalizado = normalizarCodigoNCM(ncmRaw)
  if (!normalizado) return NextResponse.json({ acuerdos: [], ntm: [] })
  ...
  const [countryRes, ntmRes] = await Promise.all([
    ...
    supabase
      ...
      .eq('hs_code', hs6)
      .eq('pais_afectado', paisISO3.toUpperCase())
      .limit(20),
  ])

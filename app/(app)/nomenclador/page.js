  ...
  return (
    <PageLayout title="NOMENCLADOR" subtitle="10.000+ posiciones arancelarias">
      <div className="max-w-5xl mx-auto">
        <div className="flex bg-surface-high rounded-lg p-1 w-fit mb-8">
          {[
            ...
            <button key={t.key} onClick={() => { setTab(t.key); setSelectedNcm(null); setCandidatos(null); }}
              className={`px-6 py-2.5 font-body text-sm font-semibold tracking-wide rounded-md transition-all duration-150 ${tab === t.key ? 'bg-surface-1 text-on-surface' : 'text-on-surface-variant hover:text-on-surface cursor-pointer'}`}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          <div>
            <div className="mb-8">
              <div className="relative max-w-2xl mx-auto">
                <svg className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-tertiary pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                  <circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" />
                </svg>
                <input className="w-full bg-surface-1 rounded-md pl-14 pr-6 py-4 text-base font-body text-on-surface placeholder:text-ink-tertiary border border-hairline outline-none focus:border-on-surface transition-all"
              ...
              <p className="font-body text-xs text-ink-tertiary text-center mt-2">Escribí un código (ej: 0902.30) o un producto (ej: galletas de chocolate)</p>
            </div>
            {!resultados && !loading && (
              <div className="flex flex-wrap justify-center gap-3 py-8">
                {CHIP_BUSQUEDAS.map(c => (<button key={c.query} className="bg-surface-1 border border-hairline rounded-full px-5 py-2.5 text-sm font-body text-on-surface-variant hover:text-on-surface hover:bg-surface-high transition-all cursor-pointer" onClick={() => handleChip(c.query)}>{c.label}</button>))}
              </div>
            )}
            {loading && resultados === null && (<div className="space-y-2">{[0,1,2,3,4,5].map(i => <div key={i} className="h-14 bg-surface-high rounded-lg animate-pulse" />)}</div>)}
            {resultados && resultados.length === 0 && (<div className="text-center py-16"><p className="font-body text-sm text-on-surface-variant">No se encontraron posiciones para "{query}"</p><p className="font-body text-xs text-ink-subtle mt-1">Probá con otro término o un código NCM más corto</p></div>)}
            {resultados && resultados.length > 0 && (
              <div>
                <div className="rounded-lg border border-hairline overflow-hidden">
                  <table className="w-full">
                    <thead><tr className="bg-surface-high">{columns.map(col => <th key={col.key} className="text-left px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-ink-subtle font-medium">{col.label}</th>)}</tr></thead>
                    <tbody>
                      {resultados.map((row, i) => (<tr key={row.codigo_ncm + i} className="border-t border-hairline hover:bg-surface-high transition-colors cursor-pointer" onClick={() => setSelectedNcm(row)}>{columns.map(col => <td key={col.key} className="px-4 py-3">{col.render ? col.render(row[col.key], row) : row[col.key]}</td>)}</tr>))}
                    </tbody>
                  </table>
                </div>
                {totalCount !== null && <p className="font-body text-xs text-ink-subtle text-center mt-4">Mostrando {resultados.length} de {totalCount.toLocaleString('es-AR')} resultados</p>}
                {hasMore && <div className="flex justify-center mt-4"><button className="bg-surface-1 border border-hairline rounded-md px-6 py-2.5 font-body text-sm text-on-surface-variant hover:text-on-surface transition-all cursor-pointer" onClick={() => buscar(query, page + 1)} disabled={loading}>{loading ? 'Cargando…' : 'Cargar más'}</button></div>}
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'clasificar' && (
          <div className="max-w-3xl mx-auto">
            <div className="bg-surface-1 rounded-lg border border-hairline p-6 mb-6">
              ...
              <div className="space-y-5">
                ...
                <div>
                  <label className="block font-body text-xs text-on-surface-variant mb-1.5">¿Cuál es su estado o procesamiento? <span className="text-red-600">*</span></label>
                  <select className="w-full bg-surface rounded-md px-4 py-3 font-body text-sm text-on-surface border border-hairline outline-none focus:border-on-surface transition-all cursor-pointer" value={clasificarForm.estado} onChange={e => setClasificarCampo('estado', e.target.value)}>
                ...
                <div>
                  ...
                  <select className="w-full bg-surface rounded-md px-4 py-3 font-body text-sm text-on-surface border border-hairline outline-none focus:border-on-surface transition-all cursor-pointer" value={clasificarForm.presentacion} onChange={e => setClasificarCampo('presentacion', e.target.value)}>
                    <option value="">Seleccionar...</option>
                ...
                <div>
                  ...
                  <textarea className="w-full bg-surface rounded-md px-4 py-3 font-body text-sm text-on-surface placeholder:text-ink-tertiary border border-hairline outline-none focus:border-on-surface transition-all resize-none" rows={2} placeholder="Peso, concentración, variedad, norma técnica, o cualquier otro dato que ayude a clasificar" value={clasificarForm.detalles} onChange={e => setClasificarCampo('detalles', e.target.value)} />
                  <p className="font-body text-[10px] text-ink-subtle mt-1">Cuanta más info, mejor la clasificación</p>
                </div>
                {clasificarError && <div className="p-3 bg-red-50 rounded-md border border-red-100"><p className="font-body text-xs text-red-600">{clasificarError}</p></div>}
            ...
            {notaHaiku && <div className="mb-6 p-4 bg-amber-50 rounded-md border border-amber-100"><p className="font-body text-sm text-amber-700">{notaHaiku}</p></div>}

            {candidatos && candidatos.length > 0 && (
              <div className="space-y-4 mb-6">
                {candidatos.map((c, i) => (
                  <div key={c.codigo_ncm} className={`bg-surface-1 rounded-lg p-5 border ${!c.ncm_exacto ? 'border-dashed border-hairline' : 'border-hairline'}`}>
                    ...
                    {c.ncm_exacto?.seccion && <p className="font-body text-xs text-ink-subtle mt-1">{c.ncm_exacto.seccion}{c.ncm_exacto.capitulo ? ` — Capítulo ${c.ncm_exacto.capitulo}` : ''}</p>}
                    <p className="font-body text-sm text-ink-subtle mt-3 italic">"{c.razonamiento}"</p>
                    {c.aranceles_impo && (
                      <div className="flex flex-wrap gap-2 mt-4 p-3 bg-surface rounded-md">
                        <div className="text-center"><span className="font-mono text-[10px] text-ink-subtle block">DIE</span><span className="font-mono text-sm text-on-surface">{c.aranceles_impo.die ?? '—'}%</span></div><div className="text-center"><span className="font-mono text-[10px] text-ink-subtle block">TE</span><span className="font-mono text-sm text-on-surface">{c.aranceles_impo.te ?? '—'}%</span></div><div className="text-center"><span className="font-mono text-[10px] text-ink-subtle block">IVA</span><span className="font-mono text-sm text-on-surface">{c.aranceles_impo.iva ?? '—'}%</span></div>
                      </div>
                    )}
                    {!c.ncm_exacto && c.similares && c.similares.length > 0 && (
                      <div className="mt-4 p-3 bg-surface rounded-md">
                        ...
                        <div className="space-y-1">{c.similares.map((s) => (<button key={s.codigo_ncm} className="w-full text-left px-3 py-2 rounded-md hover:bg-surface-high transition-colors" onClick={() => verDetalle(s)}><span className="font-mono text-xs text-primary">{formatearNCM(s.codigo_ncm)}</span><span className="font-body text-xs text-on-surface-variant ml-2">— {limpiarDescripcion(s.descripcion)?.slice(0, 50)}</span></button>))}</div>
                      </div>
                    )}
                    {(c.ncm_exacto || c.similares?.[0]) && (
                      <div className="mt-4 flex flex-col gap-2">
                        <button className="w-full bg-surface-high hover:bg-surface-2 border border-hairline rounded-md px-4 py-2.5 font-body text-sm text-on-surface transition-all cursor-pointer" onClick={() => verDetalle(c.ncm_exacto || c.similares[0])}>Ver detalle completo →</button>
                        <button className="w-full bg-primary/10 hover:bg-primary/20 border border-primary/20 rounded-md px-4 py-2.5 font-body text-sm text-primary transition-all cursor-pointer" onClick={() => router.push(`/calculadora?ncm=${c.codigo_ncm}`)}>Calcular costos con este NCM →</button>
                      </div>
                    )}
                  </div>
                ))}
            ...
            {candidatos && candidatos.length === 0 && !notaHaiku && <div className="text-center py-8"><p className="font-body text-sm text-on-surface-variant">No se encontraron candidatos para este producto.</p><p className="font-body text-xs text-ink-subtle mt-1">Probá describir mejor el producto o agregar más detalles.</p></div>}
            <div className="mt-8 p-4 bg-surface-high rounded-md border border-hairline">

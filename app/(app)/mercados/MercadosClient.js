...
export default function MercadosClient() {
  ...
  const { dolares, monedas, riesgoPais, inflacion, inflacionInteranual, tasas, commodities, indices, forex } = data || {};
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <MarketCard title="Commodities" icon={Wheat} className="xl:col-span-2">
            {commodities?.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                <div>{commodities.slice(0, 4).map(c => <CommodityItem key={c.symbol} commodity={c} />)}</div>
                <div>{commodities.slice(4).map(c => <CommodityItem key={c.symbol} commodity={c} />)}</div>
              </div>
            ) : (
        ...
        <div className="mt-6 px-4 py-3 bg-primary/[0.05] border border-primary/10 rounded-xl">
          <p className="font-body text-[11px] text-on-surface-variant leading-relaxed">
            Los datos provienen de fuentes públicas (DolarApi, ArgentinaDatos, Yahoo Finance) y pueden
            tener demoras de hasta 15 minutos. Esta información es orientativa y está respaldada por fuentes oficiales.
